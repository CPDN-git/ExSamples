...
for i in ${!ens[@]}; do
    ...
    for l in low high; do
        ...
        cdo mulc,${so2_scales[${i}]} so2dms/remapped_ukca_emiss_SO2_${l}_rcp85_2065-2076.nc so2dms/tmp/SO2_${l}.nc
    done

    echo dms_scale ${dms_scales[${i}]}
    cdo mulc,${dms_scales[${i}]} so2dms/dms_rcp45_N216_2065_2076.nc so2dms/tmp/dms.nc

    cdo merge so2dms/tmp/dms.nc so2dms/tmp/SO2_low.nc so2dms/tmp/SO2_high.nc so2dms/so2dms_r001i1p0${ens[${i}]}_2065-2076_v2.nc
