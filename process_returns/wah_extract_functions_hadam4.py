...
import sys, os, ast
import urllib.request, urllib.parse, urllib.error, tempfile, zipfile, shutil
...
def get_filename(taskpath, field,output_dir,zipstart,zipend,structure='std',zip_freq='month'):
    print('get_filename', taskpath, field,output_dir,zipstart,zipend, structure, zip_freq)
...
#        if varname[:len(field_name)]==field_name:
        if int(stash_code)==var_stash_code:
            ...
            # Check if this variable matches the required meaning period and cell method
            #if int(meaning_period)==int(var_meaning_period) and cell_method==var_cell_method:
            print('Debug: ', var_meaning_period, units, var_cell_method, meaning_period, cell_method)
...
def process_netcdf(in_ncf,out_name,field,append,zip_freq='month'):
    ...
    try:
        ...
        in_vars.sort()
        if len(in_vars)>1:
            print('Warning, multiple variables found matching request, choosing first variable',in_ncf_end, end=' ')
        in_var=in_vars[0]
        ...
        if meaning_period == 0:
            print(ntime)
            if ntime not in [120]:
                raise Exception('stopping as zero meaning period and ntime not 120')
        elif ntime != factor / (meaning_period*1.0):
        ...
        if append:
            ...
            for d in out_dims:
                ...
                if d[0] in list(nc_in_file.variables.keys()):
...
#		:exptid = "b0fn" ;
#		:field_pert = "5" ;
#		:file_atmos = "b0fn_882_atmos" ;
#		:file_ozone = "ozone_rcp45_N216L38_2009_2020v2" ;
#		:file_pert = "ic_N216_2003_03_000037.nc" ;
#		:file_sice = "sic_r001i1p02242_20091001-20100530" ;
#		:file_so2dms = "so2dms_rcp45_N216_2009_2020" ;
#		:file_solar = "solar_1985_2020" ;
#		:file_sst = "sst_r001i1p02242_20091001-20100530" ;
#		:file_sulphox = "oxi.addfa.N216L38" ;
#		:file_volc = "VOLC38_LR_N216" ;
#		:file_volcanic = "volc_1985_2020" ;
#		:ghg_filename = "ghg_defaults" ;
#		:global_stashc = "ExSamples_NH_stashc" ;
#		:model_start_month = "11" ;
#		:model_start_year = "2009" ;
#		:restart_upload_month = "0" ;
#		:run_months = "5" ;
#		:run_years = "0" ;
#		:workunit_name = "hadam4h_b0fn_200911_5_882_012035571" ;
#		:EXPTURL = "https://www.cpdn.org/workunit.php?wuid=12035571" ;
#}


            for attr in ['Conventions', 'application', 'exptid', 'batchid', 'workunit_name', 'file_pert', 'file_sst',
                         'file_sice', 'field_pert', 'file_atmos', 'file_ozone', 'file_so2dms', 'file_solar', 
                         'file_sulphox', 'file_volc', 'file_volcanic', 'ghg_filename', 'global_stashc', 'EXPTURL']:
                print(attr, nc_in_file.getncattr(attr))
                nc_out_var.UnusedNameAttribute = nc_in_file.getncattr(attr)
                nc_out_var.renameAttribute("UnusedNameAttribute", attr)

#            nc_in_var.setncattr(attr, nc_in_file.getncattr(attr))
        ...
        if append:
            ...
            for var in list(nc_out_file.variables.keys()):
                if var[:4]=='time':
                    out_time=var
...
def extract_local(taskpath, field_list, output_dir, temp_dir, zipstart, zipend):
    ...
    try:
        ...
        for nc_list in extracted_netcdfs.values():
...
#            print url
            ...
            urlret = urllib.request.urlretrieve(url, zf_fh.name)
...
def read_urls(urls_file):
    ...
    urls = fh.readlines()
    fh.close()
    urls = list(map(string.strip, urls))
    return urls
